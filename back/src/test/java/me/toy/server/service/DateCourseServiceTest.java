...
class DateCourseServiceTest {
    ...
    @Test
    @DisplayName("존재하는 데이트 코스 태그 등록 성공")
    ...
    @Test
    @DisplayName("좋아요를 누르지 않은 코스라면 좋아요 + 1")
    ...
    public void registCommentOnDateCourse() throws Exception
    {
        ...
        when(userRepository.findByEmail(userEmail)).thenReturn(Optional.of(user));
        when(dateCourseRepository.findById(dateCourse.getId())).thenReturn(Optional.of(dateCourse));
        when(commentRepository.findById(dateCourseComment.getId())).thenReturn(Optional.of(dateCourseComment));
        dateCourseService.registComment(dateCourse.getId(),comment,userEmail);
        //then
        verify(commentRepository,times(1)).save(any());
        assertThat(commentRepository.findById(dateCourseComment.getId()).get().getCommentContents()).isEqualTo(comment);
    }
}